...
async function submitForm() {
    await formData.patch(route('clients.update', { id: props.client.id }));
}
</script>
...
<template>
    <div class="container mx-auto">
        <div class="my-4">
            <h1 class="font-bold text-2xl text-center">Edit Client</h1>
        </div>
        <hr />
        <div class="my-2"></div>
        <form class="max-w-sm mx-auto" @submit.prevent="submitForm">
            ...
            <div class="text-center">
                <button type="submit" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center" :disabled="formData.processing">Edit</button>
            </div>
        </form>
    </div>
</template>
